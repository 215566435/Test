...
class payment extends React.Component {
    ...
    renderButton = () => {
        ...
        const isPrepay = () => {
            return this.props.isPrepay ? (
                <Button title={'预存款支付'} onPress={this.payByDeposite} />
            ) : null
        }
        const textStyle = {
            color: '#fa541c'
            ...
            notPay: (
                <View {...middle}>
                    <View style={{ ...centralization(), marginBottom: 150 }}>
                        ...
                        <StyledText
                            {...textStyle}
                            content={`订单号：${this.props.oi}`}
                        />
                        <StyledText
                            {...textStyle}
                            content={`支付金额：${currency + this.props.p}`}
                        />
                    </View>
                    {isPrepay()/*ji */}
                    {this.props.methods.map((m, index) => {
                        const AmountInButton =
                            this.props.p === m.money
                                ? ''
                                : `(${currency + m.money})`
                        return (
                            <View key={index}>
            ...
            fail: (
                <View {...middle}>
                    <Button
                        backgroundColor="#1890ff"
                        title={'我已完成支付，刷新结果'}
                        onPress={this.handleCheckPaymentState}
                    />
                </View>
            )
        }
...
const mapState = state => {
    console.log(state.productDetail.data)
    return {
        ...state.payment,
