    ...
    return (
      <Animated.View
        style={{
          ...
          right: 8,
          top: 48,
          transform: [{ translateY: this.state.translateY }, { scale: this.state.Scale }, { perspective: 1000 }]
        }}
      >
        <View
          style={{
        ...
        >
          {React.Children.map(this.props.children, (child, index) => (
            <View style={{ padding: 4, marginTop: 4 }} key={index}>
              {child}
            </View>
          ))}
        </View>
...
const mapDispatch = dispatch => {
  return {
    ...
    onEndEditing: inputValue => {
      dispatch({ type: 'Search', inputValue: inputValue })
    },
