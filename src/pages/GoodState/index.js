...
import { Model, PackStatus, PackStatusColor, stockState, itemStateColor, itemState } from './model'
...
class ProductDetail extends Component {
  ...
  getGoods = () => {
    ...
    const goods = go.map((item, index) => {
      const url =
        'http://cdn2u.com' + item.im + `?width=${250}` + `&height=${250}` + `&constrain=${true}` + `&bgcolor=white`
      return (
        <TouchableOpacity key={index}>
          ...
          >
            ...
            >
              <Image source={{ uri: url }} style={{ height: 90, width: 90 }} />
            </View>
            <View style={{ width: '70%', padding: 10 }}>
              ...
              >
                <StyledText content={(item.c === 'RMB' ? '¥' : '$') + item.i} color="#f56a00" fontSize="large" />
                <StyledText color="#bfbfbf" content={stockState[item.ss]} />
              </View>
              <View
                style={{
                  flexDirection: 'row',
              ...
              >
                <StyledText color={itemStateColor[item.s]} content={itemState[item.s]} />
                <StyledText color={itemStateColor[item.s]} content={`数量:${item.q}`} />
              </View>
            </View>
          </View>
        </TouchableOpacity>
        ...
        <Cells style={{ flexDirection: 'row' }}>
          {pks.map((item, index) => {
            return (
              ...
              >
                <StyledText color={item.i === i ? '#f56a00' : 'black'} content={`子订单${1 + index}`} />
              </TouchableOpacity>
            )
          })}
        </Cells>
      ...
      <View style={{ height: '100%' }}>
        ...
        >
          ...
          <JsonCell
            left={[
              //物流方式
              TextFormater(`物流方式：${u ? '现场提货' : '仓库代发'}`, 'black')
            ]}
          />
          {r ? ( //收件人
            <JsonCell
          ...
          {this.Packs()}
          <NewCell renderProps="订单历史记录" onPress={this.handleHistoryLog} />
          <NewCell onPress={this.handleAttachment} renderProps={`订单附件(${at})`} />
          {pro
            ? pro.map((item, index) => {
                const $$$ = Currency
                return (
                  <JsonCell
                    key={index}
                    left={[TextFormater(item.name, '#fa541c')]}
                    right={[TextFormater($$$ + item.price, '#fa541c')]}
                  />
                )
              })
            : null}
