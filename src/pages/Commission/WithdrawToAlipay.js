...
class WithdrawToAlipay extends Component {
  componentDidMount() {
  ...
  CustomTabBarPress = (e, child, index) => {
    ...
    if (!this.alipay) {
      Alert.alert("提交错误", "请您填写支付宝账户", [{ text: "OK" }], {
        cancelable: false
      });
      return
    }
  ...
  render() {
    console.log("提现到支付宝页面的this.props", this.props);
    return (
      <Page>
        <View
          style={{
        ...
        >
          <HeaderWithLeftArrow title="提现到支付宝" onPress={this.goBack} />
...
// 增加底部按钮
const wapprer = PageWithTab(WithdrawToAlipay, ["提交申请"], ["#f5222d"]);
