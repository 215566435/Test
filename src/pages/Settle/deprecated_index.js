...
import { header, Url, stateBarMargin, height, width, undefinedConvert } from '../../util'
    ...
    if (s) {
      return (
        ...
        >
          ...
          >
            {l
              ? `购买保险(已购买:${s}%,${(cr === 'RMB' ? '¥' : '$') + undefinedConvert(r)})`
              : `购买保险(费用:${s}%,${(cr === 'RMB' ? '¥' : '$') + undefinedConvert(r)})`}
          </Text>
        </TouchableOpacity>
      )
    }
  ...
  renderSheet = () => {
    ...
    return (
      <View>
        ...
        {dd === 0 ? null : (
          <ListItem extra={`  代发邮费:       ${convertCurrency(cr, dd)}`} ArrowColor={'transparent'} />
        )}
  ...
  render() {
    ...
    return (
      <View
        style={{
          height: '100%',
          transform: [{ translateY: this.state.isKeyboardShow ? -220 : 0 }]
        }}
      >
