...
import { PickerView, PickerCreator } from '../../components/Picker'
import WheelPicker, { ChinaArea } from '../../components/WheelPicker'
import { connect } from 'react-redux'

import { Url, header, getNavigationParam, goBack } from '../../util'
...
class Modyfiy extends Component {
    ...
    renderArea = () => {
        const d = data[this.state.privince]
            ? data[this.state.privince][this.state.city]
            : []
        return PickerCreator({
            addonBefore: '地区',
        ...
        return (
            ...
            >
                {PickerCreator({
                    addonBefore: '地址类型',
                    defaultValue:
                        this.state.type === 'Receiver' ? '收件人' : '发件人',
                ...
                {this.state.isAdd ? null : (
                    <Button title="编辑" onPress={this.onEdit} />
                )}
                <Button
                    title="保存到地址本"
