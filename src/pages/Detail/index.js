...
class DetailPage extends Component {
  ...
  shareTabPress = (e, child, index) => {
    if (index === 0) {
      ...
      console.log(urlMap)
      if (urlMap.length === 0) {
        Alert.alert('分享失败', '请选择至少一张图片进行分享', [{ text: '返回', style: 'cancel' }], {
          cancelable: false
        })
        return
      }
  ...
  renderPopup = () => {
    const PopupTabBar = () => (
      <View>
        <CustomTabBar disable={true} childColor={() => 'white'} onPress={this.PopupTabBarPress}>
          <View>
            <Text style={{ backgroundColor: 'transparent' }}>购买数量</Text>
          </View>
    ...
    return (
      <Popup isPop={this.state.isSkuSelectShow} size={400} close={this.onSkuClose} TabBar={PopupTabBar()}>
        <Pop
  ...
  render() {
    ...
    return (
      <View>
        {this.renderPopup()}
        <Modal visible={this.state.share} animationType="slide">
          <ScrollView style={{ height: height - 44 }}>
            ...
            {this.props.shareText === '' ? null : (
              <View style={{ padding: 10, marginTop: 10 }}>
                <Text style={{ color: '#108ee9', backgroundColor: 'transparent' }}>
                  分享图片后，以下文案将自动复制到手机剪切板中
                </Text>
        ...
        <CustomTabBar childColor={(child, index) => color[index]} onPress={this.tabOnPress}>
          <Ionicons name="ios-arrow-dropleft-outline" size={34} style={{ backgroundColor: 'transparent' }} />
          {btns.map((btn, index) => (
            <Text key={index} style={{ color: 'white', backgroundColor: 'transparent' }}>
              {btn}
            </Text>
          ))}
        </CustomTabBar>
...
const wrapperSpinner = () => (
  <View style={{ height: actualHeight, alignItems: 'center', justifyContent: 'center', flex: 1 }}>
    <Spin size={26} />
  </View>
)
...
const mapDispatch = dispatch => {
  return {
    ...
    addCartItem: item => {
      dispatch({ type: 'addCartItem', item: item })
    }
  }
}
