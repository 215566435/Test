...
const computedUrl = (Url) => {
    return 'http://cdn2u.com' + Url + `?width=${750}` + `&constrain=${true}` + `&bgcolor=white`
}
...
/**
 * DetailPageInstance 是用于跳转用的势力，因此我们要传递一个实例过来
 * 这样子我们才能够在alert中跳转
 */
    ...
    addCartItem: function* (state, others) {
        ...
        Alert.alert(
            ...
            [
                ...
                { text: '查看购物车', onPress: () => DetailPageInstance.props.navigation.navigate('Cart') },
            ],
            { cancelable: false }
        )
    },
